...
{if(grepl(pattern = "win", ignore.case = TRUE, x = Sys.info()[1])){
  # windows
...
ggplot(newDF2[newDF2$packedPollen == FALSE & index > 30, ], aes(x = index, y = logicalReward)) + 
  geom_point(position = position_jitter(height = 0.02), size = 2, alpha = 0.7) +
  #binomial_smooth(se = FALSE)+  
  geom_line(alpha = 0.2) + 
  facet_wrap(~ID_short) + 
...
library(gamm4)
m4 <- gamm4(logicalReward ~ s(I(index/100), by = as.factor(packedPollen)), random= ~ (1 | ID) + (1|colonyNum), family = binomial("logit"), data = newDF3)
summary(m4$mer)
summary(m4$gam)
