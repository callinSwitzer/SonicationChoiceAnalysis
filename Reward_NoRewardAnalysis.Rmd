...
packages <- c("ggplot2", "car", "plyr", "tidyr", "blmeco")
ipak(packages)
...
# clean data
newDF <- newDF[-grep("discard", tolower(newDF$Notes)), ]
# remove double reward group
newDF <- newDF[newDF$treatment != "Dev2/ai0_True__Dev2/ai1_True", ]
row.names(newDF) = 1:nrow(newDF) # renumber row names
...
newDF$packedPollen <- !(row.names(newDF) %in% grep(pattern = "corbic", tolower(newDF$Notes))) 

unique(newDF$Notes[newDF$packedPollen])

# comment out to show all bees
# newDF <- newDF3
...
summaryDF = as.data.frame(with(newDF, xtabs(~ID + rewardStatus + accNum + packedPollen)))

summaryDF

# convert to wide format
summary_wide = spread(summaryDF,   accNum, Freq)
summary_wide2 = spread(summaryDF,   rewardStatus, Freq)

# insert treatment
summary_wide$treatment = sapply(summary_wide$ID, FUN = function(x) newDF[newDF$ID == x, "treatment"][1])
...
sw3$colonyNum = sapply(sw3$ID, FUN = function(x) newDF[newDF$ID == x, "colonyNum"][1])
sw3$IT = sapply(sw3$ID, FUN = function(x) newDF[newDF$ID == x, "ITSpan_mm"][1])
...
ggplot(data=newDF2,aes(x=ID_veryShort,fill=Reward_Long)) + 
  ...
  scale_fill_viridis_d(direction = 1, option = "D", name = "") + 
  xlab("Bee ID") + 
...
# make density plots
bb = density(newDF2$index[newDF$packedPollen == TRUE])
cc = density(newDF2$index[newDF$packedPollen == FALSE])
...
dd = rbind(with(bb, 
                rbind(data.frame(x, y = y/max(y), packedPollen = TRUE), 
                      expand.grid(x = 100:0, y = 0, packedPollen = TRUE))), 
           with(cc, 
                rbind(data.frame(x, y = y/max(y), packedPollen = FALSE), 
                expand.grid(x = 100:0, y = 0, packedPollen = FALSE))))
dd <- dd[dd$x <= 100 & dd$x >= 0, ]
