...
ggplot(sw3, aes(x = substr(ID, start = 0, stop = 17), y = prop-0.005, fill = packedPollen)) + 
...
# bias reduced GLM
m1 <- glmer(cbind(true, false)~ packedPollen + (1|ID), family = binomial("probit"), data = sw4)
summary(m1)
...
ggplot(pframe, aes(x = packedPollen, y = predMean)) + 
  geom_point() + 
...
ggsave(file.path(figDir, "propSuccessPollen_25July.pdf"), width = 2, height = 4)
...
ggplot(newDF3, aes(x = propIndex, y = logicalReward)) + 
  geom_point(position = position_jitter(height = 0.1), size = 0.5) +
  binomial_smooth()+  
  geom_line(alpha = 0.2) + 
  facet_wrap(~ID) + 
