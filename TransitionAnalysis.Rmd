...
rew2 <- rewards1 %>%
  ...
  summarize(meanNo =mean(`No reward`), 
            meanRe = mean(`reward`), 
            meanQuit = mean(`quit`)) %>%
  gather(key = rewardStatus, value = propReward, meanNo:meanQuit)
rew2

ggplot(rew2, aes(x = buzzNum, y = propReward, fill = rewardStatus)) + 
  geom_bar(color = NA, position = 'fill', stat = "identity", alpha = 0.9, width=1) + 
...
buzz_msm <- msm(rewardStatus_L ~ buzzNum, subject = ID, data = rewards, 
                ...
                qmatrix = Q.crude, exacttimes = TRUE)
...
qmatrix.msm(buzz_msm, covariates = list(packedPollen = 1))
qmatrix.msm(buzz_msm, covariates = list(packedPollen = 0))
